        ...
        <div className="flex flex-col gap-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <span
                className={`ui-pill ${diagnostics.status === 'ready' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-slate-800 text-slate-500'}`}
              >
          ...
          <div className="mt-auto grid grid-cols-4 gap-3">
            <DiagnosticTile
              label="Gray Counter"
              value={diagnostics.gray}
              unit=""
              color="text-sky-400"
            />
            <DiagnosticTile
              label="Phase"
              value={diagnostics.phase?.toFixed(3)}
              unit="rad"
              color="text-indigo-400"
            />
            <DiagnosticTile
              label="Grating Mag"
              value={diagnostics.magnitude?.toFixed(0)}
              unit="px"
              color="text-emerald-400"
            />
            <DiagnosticTile
              label="Detection Score"
              value={diagnostics.score?.toFixed(2)}
              unit=""
        ...
        <div className="flex flex-col gap-6 border-l border-slate-800 pl-6">
            ...
            <div className="space-y-2">
              <div className="rounded-xl bg-black/40 p-4 border border-slate-800">
                ...
                <div className="flex items-baseline gap-1">
                  <span
                    className={`text-2xl font-mono font-black ${diagnostics.rms_ms && diagnostics.rms_ms > 25 ? 'text-rose-500' : 'text-slate-100'}`}
                  >
          ...
          <div className="mt-auto p-4 rounded-xl bg-orange-500/5 border border-orange-500/10">
            <p className="text-[10px] text-orange-200/60 leading-relaxed uppercase tracking-wider font-medium">
              The rectified view shows the crop identified by the border search. If it is "too
              zoomed in", the corners are being found incorrectly.
            </p>
          </div>
        </div>
      </div>
