...
export const useTrackingStore = create<TrackingState>((set) => ({
  ...
  addSeed: (seed) => set((state) => {
    ...
    return {
      seeds: [...state.seeds.filter(s => s.ballId !== seed.ballId || s.cameraId !== seed.cameraId), seed]
    };
  }),
  
  removeSeed: (ballId, cameraId) => set((state) => ({
    seeds: state.seeds.filter(s => s.ballId !== ballId || s.cameraId !== cameraId)
  })),
  
  startTracking: () => set({ status: 'tracking', progress: 0 }),
  
  onTrackingUpdate: (tracks, progress) => set({
    tracks,
    progress
  }),
  
  onTrackingComplete: (tracks) => set({
    tracks,
    status: 'complete',
    progress: 1.0
  }),
